"""
Python/Numeric version of this module was called mpfit.
This version was modified to use numpy.
"""
...
Perform Levenberg-Marquardt least-squares minimization, based on MINPACK-1.
        ...
        matrix is diagonally du_minant, and the iterations follow steepest
...
class MpFit:
    def __init__(self, fcn, xall=None, functkw={}, parinfo=None, ftol=1.e-10,
                 ...
                 nprint=1, iterfunct='default', iterkw={}, nocovar=0,
                 fastnorm=0, rescale=0, autoderivative=1, quiet=0, diag=None,
                 epsfcn=None, debug=0):
        """
Inputs:
fcn:
...
.covar
        ...
        The matrix is NxN where number is the number of  parameters.  The square root
...
.perror
        ...
        *If* you can assume that the true reduced chi-squared value is unity --
        ...
        # Parameters can either be stored in parinfo, or x.
        # x takes precedence if it exists
        ...
        limit = self.parinfo(parinfo, 'limit', default=[0., 0.], n=npar)

        if (limited != None) and (limit != None):
            # Error checking on limit in parinfo
            wh = numpy.nonzero((limited[:, 0] & (xall < limit[:, 0])) |
                               (limited[:, 1] & (xall > limit[:, 1])))
            if (len(wh[0]) > 0):
                self.errmsg = 'ERROR: parameters are not within PARINFO limit'
                return
            wh = numpy.nonzero((limited[:, 0] & limited[:, 1]) &
                                                          (limit[:, 0] >= limit[:, 1]) &
                                                          (pfixed == 0))
            if (len(wh[0]) > 0):
                self.errmsg = 'ERROR: PARINFO parameter limit are not consistent'
                return

            # Transfer structure values to local variables
            qu_lim = numpy.take(limited[:, 1], ifree)
            u_lim = numpy.take(limit [:, 1], ifree)
            qllim = numpy.take(limited[:, 0], ifree)
            llim = numpy.take(limit [:, 0], ifree)

            wh = numpy.nonzero((qu_lim != 0.) | (qllim != 0.))
            if (len(wh[0]) > 0): qanylim = 1
            else: qanylim = 0
        else:
            # Fill in local variables with dummy values
            qu_lim = numpy.zeros(nfree, dtype=numpy.int8)
            u_lim = x * 0.
            qllim = qu_lim
            llim = x * 0.
            qanylim = 0
        ...
        while(1):
            ...
            fjac = self.fdjac2(fcn, x, fvec, step, qu_lim, u_lim, dside,
                                        epsfcn=epsfcn,
            ...
            # Determine if any of the parameters are pegged at the limit
            if (qanylim):
                catch_msg = 'zeroing derivatives of pegged parameters'
                whlpeg = (numpy.nonzero(qllim & (x == llim)))[0]
                nlpeg = len(whlpeg)
                whupeg = (numpy.nonzero(qu_lim & (x == u_lim)))[0]
                nupeg = len(whupeg)
            ...
            while(1):
                ...
                [fjac, par, wa1, wa2] = self.lmpar(fjac, ipvt, diag, qtf, delta, wa1,
                                                   wa2, par=par)
                # Store the direction p and x+p. Calculate the norm of p
                wa1 = -wa1

                if (qanylim == 0) and (qminmax == 0):
                    # No parameter limit, so just move to new position WA2
                    alpha = 1.
                    wa2 = x + wa1

                else:

                    # Respect the limit.  If a step were to go out of bounds, then
                    ...
                    if (qanylim):
                        ...
                        whu = (numpy.nonzero(((dwa1 != 0.) & qu_lim) & ((x + wa1) > u_lim))
                               )[0]
                        if (len(whu) > 0):
                            t = ((numpy.take(u_lim, whu) - numpy.take(x, whu)) /
                                            numpy.take(wa1, whu))
                            alpha = min(alpha, min(t))
                    ...
                    wh = (numpy.nonzero((qu_lim != 0.) & (wa2 >= u_lim * (1 - machep))))[0]
                    if (len(wh) > 0): numpy.put(wa2, wh, numpy.take(u_lim, wh))
    ...
    def fdjac2(self, fcn, x, fvec, step=None, u_limited=None, u_limit=None, dside=None,
        ...
        if len(u_limited) > 0 and len(u_limit) > 0:
            #mask = mask or (u_limited and (x > u_limit-h))
            mask = mask | (u_limited & (x > u_limit - h))
    ...
    # via (I - 2 vT . v / (v . vT)).
    #
    #   hal = ident                                    # identity matrix
    #   for i = 0, n-1 do begin
    #    v = aa(*,i) & if i GT 0 then v(0:i-1) = 0    # extract reflector
    #    hal = hal # (ident - 2*(v # v)/total(v * v))  # generate matrix
    #   endfor
    #
    # Test the result:
    #    IDL> print, hal # transpose(r)
    #          9.00000      4.00000
    #          2.00000      8.00000
    #          6.00000      7.00000
    #
